...
설계 초안에 대해 다음 전문가 관점을 순차 또는 병렬로 적용하여 검토합니다:

**필수 검토자** (최소 2명, 최대 4명):
- **보안 전문가**: 인증/인가, 데이터 보호, 취약점
- **성능 전문가**: DB 인덱스 전략, 캐싱, 쿼리 최적화
- **UX 전문가**: 사용자 흐름, 에러 처리, 접근성 (UI 포함 기능 시)
- **인프라 전문가**: 확장성, 배포 위험도, 롤백 전략 (인프라 변경 시)
