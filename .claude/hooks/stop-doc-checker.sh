...
# sprint-close: ROADMAP.md + CHANGELOG.md 동시 수정
elif echo "$CHANGED_FILES" | grep -q "^ROADMAP\.md$" && echo "$CHANGED_FILES" | grep -q "^CHANGELOG\.md$"; then
  AGENT="sprint-close"

# sprint-planner: docs/sprint/sprint{N}.md 생성
elif echo "$CHANGED_FILES" | grep -qE '^docs/sprint/sprint[0-9]+\.md$'; then
