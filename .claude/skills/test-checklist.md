...
수동 항목(`⬜`)이 남은 상태에서 다음 단계 진행 가능 여부:

| 수동 항목 | 미완료 시 PR/배포 차단 여부 | 근거 |
|----------|---------------------------|------|
...
**공통 규칙**: 수동 항목이 `⬜`인 상태에서 PR 생성은 가능하나, 모든 수동 항목이 `✅`로 완료되기 전까지 merge/배포를 진행하지 않는다. Critical 보안 이슈와 관련된 수동 항목은 반드시 완료 후 merge한다.
